packages=c("stringr","qvalue","vegan","tidyr","moments","limma","dplyr","gplots",
           ...
           "matrixStats","pacman","fgsea","RColorBrewer") 
...
weightedKS.batch<-function(weight,feature.list,preCal=TRUE,high.enrich=TRUE,p.cut=0.05,correct.outlier=FALSE,minsize=5){
  ...
  ks.result<-weightedKSV2(weights=tmp.weight,compare.list=feature.list,myPermu=myPermu,p.cut=p.cut,min.numOnList=minsize)
  return(ks.result)
}
...
run.weightedKS<-function(weight,signed=TRUE,feature.list,minsize=10,correct.outlier=FALSE,transformNegWeight=FALSE){
  ...
  if (signed==TRUE){
    ...
    ks.descending<-weightedKS.batch(weight=posWeight,feature.select,preCal = TRUE,high.enrich = TRUE,correct.outlier=correct.outlier,minsize=minsize)
    ks.ascending<-weightedKS.batch(weight=negWeight,feature.select,preCal = TRUE,high.enrich = TRUE,correct.outlier=correct.outlier,minsize=minsize) 
  }else{
    ks.descending<-weightedKS.batch(weight,feature.select,preCal = TRUE,high.enrich = TRUE,correct.outlier=correct.outlier,minsize=minsize)
    ks.ascending<-weightedKS.batch(weight,feature.select,preCal = TRUE,high.enrich = FALSE,correct.outlier=correct.outlier,minsize=minsize) 
  }
  ks.all=list(ks.descending=ks.descending,ks.ascending=ks.ascending)
  return(ks.all)
}
####Calculate uniConSig scores based on ks test results # default parameters: p.cut=0.01,q.cut=0.25,NES.cut=0,power=1,root=1,ECNpenalty=0.5,
cal.uniConSig.ks<-function(up.ks,down.ks,preCalmatrix,feature.list,outfile,p.cut=0.01,q.cut=0.25,NES.cut=0,power=1,root=1,ECNpenalty=0.5){
  up.ks[is.na(up.ks)] <- 0
...
weightedKSV2<-function(weights,compare.list,myPermu,p.cut=0.05,min.numOnList=5,cal.qValue=T){ 
...
GSEA<-function(target.score,compare.list,p.cut=0.05,minsize=5,min.numOnList=5){
  if (!exists("compare.list")){
    stop("please provide the list of concepts to variable: feature.list")
  }
  tmp.weight=normalize(target.score)
  tmp.weight=tmp.weight[which(tmp.weight!=0)]
  if (length(tmp.weight)<50){
    stop(paste("There are only",length(tmp.weight),"genes have uniConSig scores, cannot perform CSEA")) 
  }else if(length(tmp.weight)<1000){
    print (paste("There are only",length(tmp.weight),"genes have weights, CSEA results may not be reliable"))
  }else {
    print (paste("There are",length(tmp.weight),"genes have weights"))
  }
  compare.list=lapply(compare.list,filter,filter=names(tmp.weight),inlist=TRUE)
  compare.list<-compare.list[which(sapply(compare.list,function(x) length(x)>=minsize))]
  matchn=sapply(compare.list,function(x){length(intersect(names(tmp.weight),x))})
...
disambiguation.CSEA<-function(GEA.result,uniConSig.result,compare.list,upPathways=NULL,topn=30,p.cut=0.05,p.adjust.method="BH"){ #upPathways TRUE or FALSE (for W-CSEA) or NULL (NULL is for D-CSEA). p.adjust.method-- chose one: NULL or "bonferroni" or "BH"
...
disambiguation.GSEA<-function(GEA.result,weight,compare.list,topn=30,p.cut=0.05,transformNegWeight=FALSE,p.adjust.method="BH"){ #p.adjust.method-- chose one: NULL or "bonferroni" or "BH"
  topPathway=GEA.result$Compare.List[1:topn]
  topPathway.list=compare.list[topPathway]
  topPathway.del=list()
  for (i in 1:length(topPathway.list)){
...
limmaDGE<-function(gctFile,clsFile,groups.order=NULL){ # the comparison is based on increasingly sorted orders of group labels with positive t value showing upregulated in class 1
  expData=read.table(gctFile, stringsAsFactors=F, skip=2, header=T, row.names=NULL, check.names=F, fill=TRUE,sep="\t")
  expUnique<-expData[which(!duplicated(expData[,1])),]
  rownames(expUnique)<-expUnique[,1]
  expUnique=expUnique[,-c(1,2)]
  ...
  if (typeof(groups.order)=="NULL"){
    myClass<-factor(myClass[2,],levels=myClass[1,2:3])
  }else{
    myClass<-factor(myClass[2,],levels=groups.order)
  }
...
#draw interactome network for a selected pathway based on DGE weights
...
##### ======== ##### ======== ##### ======== SCDE and calculate signed Q-value  ##### ======== ##### ======== ##### ======== 
library(scde)
scdeDEG<-function(readCountGCT=readCountGCTFile,clsFile=clsFile, groups.order="NULL", min.lib.size=100, min.reads=1,min.detected=1,SignPzscore=0.75)  {
  countData=read.table(readCountGCT, stringsAsFactors=F, skip=2, header=T, row.names=NULL, check.names=F, fill=TRUE,sep="\t")
  countUnique<-countData[which(!duplicated(countData[,1])),]
  ...
  myClass=read.table(clsFile, sep="\t", stringsAsFactors=F, header=F, skip=1, row.names=NULL, check.names=F, fill=TRUE) #skip=1,
  if (typeof(groups.order)=="NULL"){
    myClass<-factor(myClass[2,],levels=myClass[1,2:3])
  }else{
    myClass<-factor(myClass[1,],levels=groups.order)
  }
  myClass_TP <- t(myClass)
  myClass<-as.factor(myClass_TP[1,])
